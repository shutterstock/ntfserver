...
reports['test-duration'] = function(req, cb) {
  ...
  if (isNaN(req.test_id) || typeof(req.test_id) !== 'number') {
  ...
  args.push(req.test_id)
  where = plusWhere(where, 'test_id = ?')

  shared.sql.query(
    'SELECT' +
    '  FROM_UNIXTIME(ROUND(UNIX_TIMESTAMP(create_time) / ' + period + ') * ' + period + ') AS time,' +
    '  SUM(duration) / COUNT(*) AS duration' +
    '  FROM test_result' +
    '  ' + where +
    '  GROUP BY ROUND(UNIX_TIMESTAMP(create_time) / ' + period + ')' +
  ...
  function(err, data) {
    ...
    cb(null, { period: period, data: data })
  })
}
