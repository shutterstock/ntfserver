...
reports['test-duration'] = function(req, cb) {
  req.period = parseInt(req.period || 3600)
  req.test_id = parseInt(req.test_id)

  if (isNaN(req.period))
    return cb(new Error('period must be a number'))
  if (isNaN(req.test_id))
    return cb(new Error('test_id must be a number'))

  var args = [req.test_id]
  var where = plusWhere(null, 'test_id = ?')

  shared.sql.query(
    'SELECT' +
    '  FROM_UNIXTIME(ROUND(UNIX_TIMESTAMP(create_time) / ' + req.period + ') * ' + req.period + ') AS time,' +
    '  SUM(duration) / COUNT(*) AS duration' +
    '  FROM test_result' +
    '  ' + where +
    '  GROUP BY ROUND(UNIX_TIMESTAMP(create_time) / ' + req.period + ')' +
    '  ORDER BY create_time ASC', args,
  function(err, data) {
    if (err) return cb(err)
    ...
    cb(null, { period: req.period, data: data })
  })
}
