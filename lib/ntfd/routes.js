...
module.exports = function(app) {
  var maxAge = function(req) {
    var cacheControl = req.header('cache-control')
      , timeout = global.options.test.timeout * 2
  ...
  app.get('/', function(req, res) {
    helper.runTests(global.suite, { maxAge: global.options.test.timeout * 2 }, function(err, data) {
      res.render('index.html', { data: data })
    })
  })

  app.get('/test', function(req, res) {
    var data = Object.keys(global.suite)
    data.sort()
  ...
  app.get('/test/all', function(req, res) {
    helper.runTests(global.suite, { maxAge: maxAge(req) }, function(err, data) {
  ...
  app.get('/test/:name', function(req, res) {
    ...
    if (!global.suite[name]) return res.send(404)
    helper.runTest(name, global.suite[name], { maxAge: maxAge(req) }, function(err, data) {
