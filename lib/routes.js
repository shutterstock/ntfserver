      ...
      if (obj.test_name) {
        if (testList.length) {
          if (json(req)) {
            ...
            context.test.report_url = '/reports/test-duration?test_id=' + context.test.id
            if (req.url.indexOf('?') >= 0) context.test.report_url += '&' + req.url.slice(req.url.indexOf('?') + 1)
            res.render('suites/tests/detail.html', context)
          }
        } else {
          res.send(404)
        }
