...
exports.setup = function(app) {
  ...
  app.get('/suites/:suite_name/results/:suite_result_id?', function(req, res) {
    ...
    api.getSuiteResultList(list(req, obj), function(err, result) {
      if (err) return next(err)
      if (obj.suite_result_id) {
        if (result.length) {
          if (json(req)) {
            ...
            api.getSuiteList({ suite_id: result[0].suite_id }, function(err, suite) {
              if (err) return next(err)
              res.render('suites/results/detail.html', { result: result[0], suite: suite[0] })
            })
          }
        } else {
          res.send(404)
        ...
        if (json(req)) {
          ...
          api.getSuiteList({ suite_id: result[0].suite_id }, function(err, suite) {
            res.render('suites/results/index.html', { results: result, suite: suite[0] })
          })
        }
      }
    })
