  ...
  app.get('/', function(req, res, next) {
    if (json(req)) {
      ...
      api.getEventList(function(err, eventList) {
        ...
        var refresh = parseInt(req.query.refresh || 60)
        refresh = refresh > 30 ? refresh : 30
        res.render('index.html', { fail: fail, ok: !fail, refresh: refresh })
      })
    }
  })
  ...
  app.get('/events', function(req, res, next) {
    api.getEventList(function(err, result) {
      ...
      if (json(req)) {
        ...
        var refresh = parseInt(req.query.refresh || 60 )
        refresh = refresh > 30 ? refresh : 30
        res.render('events.html', { events: result, refresh: refresh })
      }
    })
  })
  ...
  app.get('/status', function(req, res, next) {
    api.getStatus(function(err, result) {
      ...
      if (json(req)) {
        ...
        var refresh = parseInt(req.query.refresh || 60 )
        refresh = refresh > 30 ? refresh : 30
        res.render('status.html', { results: result, refresh: refresh })
      }
    })
  })
